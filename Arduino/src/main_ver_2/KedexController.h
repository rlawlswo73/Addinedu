...
#include "Product.h"
#include <Stepper.h>
#include "Queue.h"  // Queue 라이브러리

#define MAX_QUEUE_SIZE 10  // 큐의 최대 크기 설정
  ...
  public:
    ...
    void push(String data) {
      if (size < MAX_QUEUE_SIZE) {
        queue[rear] = data;
        rear = (rear + 1) % MAX_QUEUE_SIZE;  // 원형 큐
        size++;
      }
    }
    ...
    String pop() {
      if (size > 0) {
        ...
        front = (front + 1) % MAX_QUEUE_SIZE;  // 원형 큐
        size--;
        return data;
      }
